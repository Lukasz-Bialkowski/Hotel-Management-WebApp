/**
 * CRUD Filter/Sort REST service
 * 
 * @constructor
 * @param {string}
 *            baseUrl - REST base URL, eg. http://localhost:8080/api/
 * @param {Object}
 *            $resource - Angular $resource service
 * @param {string}
 *            cls - REST route, eg. accounts, will result with
 *            http://localhost:8080/api/accou8nts
 */
function CrudfsSrv(baseUrl, $resource, cls) {
    this.operations = {
        list : {
            method : 'GET',
            isArray : true
...
// FIXME Support of UI without view and custom selected item scope name
function CrudfsCtrl($scope, crudSrv, confirmSrv, rmMsg, noInit) {
    ...
    $scope.getById = function(id, callback) {
        crudSrv.get({
            id : id
        }, function(response) {
    ...
    $scope.removeBack = function(item, backRef, backParams) {
        confirmSrv.confirm(function() {
            crudSrv.remove({
                id : item.id
            }, function(response) {
                $scope.$state.go(backRef, backParams);
            }, function(response) {
            });
